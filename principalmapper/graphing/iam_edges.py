...
class IAMEdgeChecker(EdgeChecker):
    ...
    def return_edges(self, nodes: List[Node], region_allow_list: Optional[List[str]] = None,
        ...
        f = functools.partial(generate_edges_locally, dest_nodes=nodes, scps=scps)
        for result in self.pool.map(f, nodes, chunksize=20):
            results.extend(result)

            for edges in result:
                logger.info("Found new edge: {}\n".format(edges.describe_edge()))

        return results


def generate_edges_locally(node_source: List[Node], dest_nodes: List[Node], scps: Optional[List[List[dict]]] = None) -> \
List[Edge]:
    """Generates and returns Edge objects. It is possible to use this method if you are operating offline (infra-as-code).
    """
    result = []

    for node_destination in dest_nodes:
